...
export default function AdminLayout({ activeTab, onTabChange, children }: AdminLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
      ...
      <aside
        className={`
          fixed top-0 left-0 z-50 h-full bg-white border-r border-slate-200 shadow-xl
      ...
      >
        {/* Sidebar Header */}
        <div className="flex items-center justify-between h-16 px-4 border-b border-slate-200 bg-gradient-to-r from-blue-600 to-blue-700">
        ...
        {/* User Info */}
        <div className={`p-4 border-b border-slate-100 ${sidebarCollapsed ? 'lg:p-2' : ''}`}>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <div className={`text-left ${sidebarCollapsed ? 'lg:hidden' : ''}`}>
        ...
        {/* Bottom Actions */}
        <div className={`p-3 border-t border-slate-200 space-y-1 ${sidebarCollapsed ? 'lg:p-2' : ''}`}>
          <Link
            href="/dashboard"
            className={`
              w-full flex items-center space-x-3 px-3 py-3 rounded-xl text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-all duration-200
              ${sidebarCollapsed ? 'lg:justify-center lg:px-2' : ''}
            `}
          >
            <Home size={20} className="text-slate-500 flex-shrink-0" />
            <span className={`font-medium ${sidebarCollapsed ? 'lg:hidden' : ''}`}>Go to Service</span>
          </Link>
          <button
            onClick={logout}
            className={`
              w-full flex items-center space-x-3 px-3 py-3 rounded-xl text-red-600 hover:bg-red-50 transition-all duration-200
              ${sidebarCollapsed ? 'lg:justify-center lg:px-2' : ''}
            `}
          >
