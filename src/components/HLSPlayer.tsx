...
export default function HLSPlayer({ src, poster = '/poster.jpg' }: HLSPlayerProps) {
  ...
  return (
    ...
    >
      ...
      {!player.loading && !player.isActive && !player.error && (
        <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-gray-800 to-gray-900 text-white">
          <div className="text-center px-4 sm:px-6">
            ...
            {player.nextScheduled && (
              <div className="mt-4 space-y-2">
                <p className="text-xs sm:text-sm text-gray-300 font-medium">
                  Next: {player.nextScheduledLabel || 'Service'}
                </p>
                <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-white/10 rounded-full text-xs sm:text-sm font-semibold text-white">
                  ...
                  {player.countdown || 'Starting soon'}
                </div>
              </div>
            )}
          </div>
      ...
      {player.streamOffline && player.isActive && !player.error && (
      ...
      {player.error && (
        <div className="absolute inset-0 flex items-center justify-center bg-red-900 text-white p-3 sm:p-4">
          <div className="text-center">
            ...
            <div className="text-xs sm:text-sm max-w-xs">{player.error}</div>
          </div>
        </div>
      )}
      ...
      <video
        ref={videoRef}
        className={`w-full h-full ${(!player.isActive && !player.loading) || player.streamOffline ? 'hidden' : ''}`}
        preload="metadata"
      ...
      {/* Custom Play Overlay */}
      {!player.isPlaying && !player.error && !player.loading && player.isActive && !player.streamOffline && (
        <div
      ...
      {player.isActive && !player.streamOffline && (
      ...
      {player.isActive && !player.streamOffline && !player.error && !player.loading && (
        <div 
          className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent transition-opacity duration-300 ${player.showControls || !player.isPlaying ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        >
          {/* Progress Bar - only show for non-live/seekable content */}
          {player.duration > 0 && isFinite(player.duration) && (
            <div 
              ref={progressRef}
              className="relative h-1 hover:h-1.5 mx-4 mb-2 cursor-pointer group/progress transition-all"
              onClick={player.handleProgressClick}
            >
              {/* Background */}
          ...
          <div className="flex items-center gap-2 sm:gap-4 px-4 pb-3">
            ...
            <div className="flex items-center gap-2 group/volume">
              ...
              <input
                ...
                onChange={player.handleVolumeInput}
