    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <header className="bg-gradient-to-r from-blue-800 to-blue-900 shadow-lg">
        <div className="max-w-screen-2xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center py-3 sm:py-5 gap-2 sm:gap-0">
            <div className="min-w-0 flex-1">
              <h1 className="text-lg sm:text-2xl font-bold text-white truncate">{serviceSettings.headerTitle}</h1>
              <p className="text-xs sm:text-sm text-blue-100 truncate">
                {user ? (
                  ...
                  <>
                    Welcome to <span className="font-semibold">{activeEvent.title}</span>
                  </>
                ) : (
                  <>Guest</>
                )}
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
              {user && user.role === 'ADMIN' && (
                <Link
                  ...
                  className="bg-white hover:bg-blue-50 text-blue-800 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors shadow-sm"
                >
                  <span className="hidden sm:inline">Go to </span>Admin
                </Link>
              )}
              {user && (
                <button
                  onClick={logout}
                  className="bg-red-500 hover:bg-red-600 text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors shadow-sm"
                >
                  Logout
                </button>
              )}
      ...
      <main className="max-w-screen-2xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
        {/* Video Player + Rhapsody Side Panel */}
        <div className="flex flex-col lg:flex-row gap-3 sm:gap-4 lg:gap-6 mb-4 sm:mb-6 lg:mb-8">
          {/* Video Player - Takes 2/3 width on desktop */}
          <div className="lg:w-2/3">
            <div className="bg-black rounded-lg sm:rounded-xl shadow-xl overflow-hidden">
          ...
          <div className="lg:w-1/3 bg-white rounded-lg sm:rounded-xl shadow-lg flex flex-col overflow-hidden border border-gray-200 min-h-[320px] sm:min-h-[400px] lg:min-h-0 lg:max-h-[calc((100vw-3rem)*2/3*9/16)] xl:max-h-[calc((min(100vw,1536px)-3rem-3rem)*2/3*9/16)]">
            <div className="bg-gradient-to-r from-blue-700 to-blue-800 px-3 sm:px-4 py-2.5 sm:py-3 flex-shrink-0">
              <h3 className="text-sm sm:text-base font-semibold text-white text-center">
                Today in Rhapsody of Realities
              </h3>
            </div>
            <div className="px-3 sm:px-4 py-3 sm:py-4 flex-1 min-h-0 overflow-y-auto">
            ...
            {hasUpcomingSchedules ? (
              upcomingSchedules.map((schedule, index) => {
                ...
                return (
                  <div
                    key={schedule.id}
                    className={`bg-white rounded-lg sm:rounded-xl shadow-md p-3 sm:p-5 border-l-4 ${colors.border} hover:shadow-lg transition-shadow ${
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <div className={`text-base sm:text-xl font-bold ${colors.text} truncate`}>{schedule.name}</div>
                        <div className="text-xs sm:text-sm text-gray-600 mt-0.5 sm:mt-1">{schedule.time}</div>
                        <div className="text-xs text-gray-500 mt-0.5 sm:mt-1">
                          {formatNextDate(schedule.nextDate)}
                        </div>
                      </div>
                      <div className={`w-10 h-10 sm:w-12 sm:h-12 ${colors.iconBg} rounded-full flex items-center justify-center ${colors.text} flex-shrink-0`}>
                        {icon}
                      </div>
                    </div>
                  </div>
              ...
              <>
                <div className="bg-white rounded-lg sm:rounded-xl shadow-md p-3 sm:p-5 border-l-4 border-blue-700 hover:shadow-lg transition-shadow">
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="text-base sm:text-xl font-bold text-blue-800 truncate">{serviceSettings.sundayLabel}</div>
                      <div className="text-xs sm:text-sm text-gray-600 mt-0.5 sm:mt-1">{serviceSettings.sundayTime}</div>
                    </div>
                ...
                <div className="bg-white rounded-lg sm:rounded-xl shadow-md p-3 sm:p-5 border-l-4 border-blue-600 hover:shadow-lg transition-shadow">
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="text-base sm:text-xl font-bold text-blue-800 truncate">{serviceSettings.wednesdayLabel}</div>
                      <div className="text-xs sm:text-sm text-gray-600 mt-0.5 sm:mt-1">{serviceSettings.wednesdayTime}</div>
                    </div>
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0">
                ...
                <div className="bg-white rounded-lg sm:rounded-xl shadow-md p-3 sm:p-5 border-l-4 border-blue-500 hover:shadow-lg transition-shadow sm:col-span-2 lg:col-span-1">
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="text-base sm:text-xl font-bold text-blue-800 truncate">{serviceSettings.prayerLabel}</div>
                      <div className="text-xs sm:text-sm text-gray-600 mt-0.5 sm:mt-1">{serviceSettings.prayerTime}</div>
                    </div>
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0">
        ...
        {activeBanners.length > 0 && (
          <div className="bg-white rounded-lg sm:rounded-xl shadow-lg p-3 sm:p-5 lg:p-6">
            <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4">Announcements</h3>
